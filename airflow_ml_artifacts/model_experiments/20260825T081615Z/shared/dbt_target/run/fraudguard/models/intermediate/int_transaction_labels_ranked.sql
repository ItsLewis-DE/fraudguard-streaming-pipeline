  ...
  create or replace view `fraudguard_intermediate`.`int_transaction_labels_ranked` 
  ...
  as (
    ...
    select l.*
    from `fraudguard_staging`.`stg_transaction_labels` l
    inner join `fraudguard_intermediate`.`int_committed_ingestion_batches` b
